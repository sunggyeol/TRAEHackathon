...
const NO_EMOJI_RULE = '응답 시 이모지(emoji)를 절대 사용하지 마세요. 마크다운 문법(**굵게**, ##제목, |테이블| 등)도 사용하지 마세요. 순수 텍스트와 번호 목록(1. 2. 3.)만 사용하세요. 표 형태의 데이터는 "- 항목: 값" 형식의 목록으로 작성하세요.';
...
응답 시 이모지 사용을 엄격히 금지하며, 깔끔하고 간결한 마크다운(bold, list 등)만 사용하세요.
${body.context ? `\n현재 데이터 컨텍스트:\n${body.context}` : ''}`;
...
const WORKFLOWS: Record<string, AgentDef[]> = {
  ...
  unify: [
    { id: 'schema-validator', name: '스키마 검증 에이전트', emoji: '🔍',
      system: '당신은 다양한 이커머스 플랫폼의 정산 데이터 스키마를 검증하는 AI 에이전트입니다. 각 플랫폼의 데이터가 통합 스키마(order_id, order_date, product_name, quantity, sales_amount, commission, settlement, platform)에 올바르게 매핑되었는지 검증합니다. 누락 필드, 데이터 타입 불일치, 이상값을 탐지합니다. 한국어로 3-4문장으로 핵심만 간결하게 답변하세요.',
      prompt: (ctx) => `다음 통합 데이터의 스키마 정합성을 검증하세요. 누락 필드, 이상값, 플랫폼별 데이터 품질을 점검하세요:\n${ctx}` },
    { id: 'dedup-detector', name: '중복 탐지 에이전트', emoji: '🔄',
      system: '당신은 이커머스 정산 데이터에서 중복 주문, 충돌 레코드를 탐지하는 AI 에이전트입니다. 동일 주문번호의 중복, 플랫폼 간 동일 상품의 가격 불일치, 날짜 범위 겹침 등을 분석합니다. 한국어로 3-4문장으로 핵심만 간결하게 답변하세요.',
      prompt: (ctx) => `다음 통합 데이터에서 중복 주문, 데이터 충돌, 불일치를 탐지하세요:\n${ctx}` },
    { id: 'export-formatter', name: '통합 포맷 에이전트', emoji: '📋',
      system: '당신은 여러 플랫폼의 정산 데이터를 하나의 표준화된 형식으로 통합하는 AI 에이전트입니다. 통합 파일의 컬럼 구성, 정렬 기준, 요약 통계를 제안합니다. 한국어로 3-4문장으로 핵심만 간결하게 답변하세요.',
      prompt: (ctx) => `다음 데이터를 기반으로 통합 CSV 파일의 최적 구성을 제안하세요. 컬럼 순서, 정렬 기준, 포함할 요약 행을 추천하세요:\n${ctx}` },
  ],
};
...
const SYNTH_PROMPTS: Record<string, string> = {
  ...
  risk: '당신은 여러 리스크 에이전트의 진단 결과를 종합하는 리스크 관리자입니다. 각 리스크의 심각도(높음/중간/낮음)를 평가하고, 즉시 조치가 필요한 항목을 식별하세요. 각 리스크를 번호 목록으로 정리하고, 각 항목에 심각도와 조치 필요 여부를 명시하세요. 응답 시 이모지 사용을 엄격히 금지하며, 볼드나 리스트 등의 마크다운 문법도 사용하지 마세요. 한국어로 답변하세요.',
